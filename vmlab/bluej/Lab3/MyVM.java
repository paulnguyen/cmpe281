...
public class MyVM
{
    ...
    public void unresgister() 
    {
        try {
            ...
            if ( vm != null )
            {
                System.out.println( "Unregistering " + vmname ) ;
                vm.unregisterVM();
            }        
        }
    ...
    public void register()
    {
        try {
            ...
            Datacenter dc = (Datacenter) new InventoryNavigator(rootFolder).searchManagedEntity("Datacenter", Config.getDataCenter() );
            HostSystem host = (HostSystem) new InventoryNavigator(rootFolder).searchManagedEntity("HostSystem", Config.getHost() );
            Folder vmFolder = dc.getVmFolder();
            ResourcePool rp = null ;
            ManagedEntity [] rps = new InventoryNavigator(dc).searchManagedEntities( "ResourcePool" ) ; 

            for(int i=0; i<rps.length; i++)
            {
                if ( rps[i].getName().equals( Config.getResourcePool() ) )
                    rp = (ResourcePool) rps[i] ;                 
            }

            if ( rp != null )
            {
                System.out.println( "Registering " + vmname ) ;
                Task task = vmFolder.registerVM_Task( Config.getVmConfigFile(),Config.getVmwareVM(),false,rp,host);
                while(task.getTaskInfo().getState()==TaskInfoState.running)
                {
                    System.out.println( "Registration in progress..." );
                    Thread.currentThread().sleep(3000);
                }
                System.out.println( "Registration Completed " + task.getTaskInfo().getState() );
            }            
        }
        catch( Exception e ) { System.out.println( e.getMessage() ) ; }
    }
