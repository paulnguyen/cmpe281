...
func gumballMachineUpdateWorker() {
	for {
		chg := <- order_update
		fmt.Println("Updating Machine: ", chg)
		cnt := machine.CountGumballs
		cnt = cnt + chg		
		machine.CountGumballs = cnt
		fmt.Println("Updated Machine: ", cnt)
	}
}
